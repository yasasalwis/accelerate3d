import {Box, Clock, FileCode, Maximize2} from "lucide-react"
...
const getMaterialColor = (material: string | null | undefined) => {
    ...
    if (mat.includes('PLA')) return {
        bg: "bg-green-500/10",
        text: "text-green-400",
        border: "border-green-500/30",
        label: "PLA"
    }
    if (mat.includes('ABS')) return {
        bg: "bg-orange-500/10",
        text: "text-orange-400",
        border: "border-orange-500/30",
        label: "ABS"
    }
    if (mat.includes('PETG')) return {
        bg: "bg-blue-500/10",
        text: "text-blue-400",
        border: "border-blue-500/30",
        label: "PETG"
    }
    if (mat.includes('TPU')) return {
        bg: "bg-purple-500/10",
        text: "text-purple-400",
        border: "border-purple-500/30",
        label: "TPU"
    }
    if (mat.includes('NYLON') || mat.includes('PA')) return {
        bg: "bg-cyan-500/10",
        text: "text-cyan-400",
        border: "border-cyan-500/30",
        label: "NYLON"
    }
    if (mat.includes('ASA')) return {
        bg: "bg-yellow-500/10",
...
export function ModelCard({
    ...
    return (
        <div className={cn(
            ...
            {/* Thumbnail */}
            <div
                className="relative aspect-video w-full rounded-lg bg-black/40 overflow-hidden mb-4 border border-white/5">
                {thumbnailUrl ? (
                ...
                <div className="absolute bottom-2 left-2 right-2 flex justify-between items-center gap-1.5">
                    <span
                        className="text-[10px] font-mono text-zinc-400 bg-black/60 px-1.5 py-0.5 rounded border border-white/5">
                        {dimensions.width}x{dimensions.depth}x{dimensions.height} mm
                    </span>
                    <div className="flex gap-1.5">
                        <span
                            className={cn("text-[10px] font-mono font-bold px-1.5 py-0.5 rounded border", materialStyle.bg, materialStyle.text, materialStyle.border)}>
                            {materialStyle.label}
                        </span>
                        <span
                            className="text-[10px] font-mono text-neon-lime bg-neon-lime/10 px-1.5 py-0.5 rounded border border-neon-lime/20">
                            {filamentGrams}g
                        </span>
                    </div>
                </div>
